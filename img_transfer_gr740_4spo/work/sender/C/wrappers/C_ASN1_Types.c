...
#ifdef __NEED_RawData_UPER
int Encode_UPER_RawData(void *pBuffer, size_t iMaxBufferSize, const asn1SccRawData *pSrc)
{
    (void)iMaxBufferSize;
    int errorCode;
    STATIC BitStream strm;
    ...
    BitStream_Init(&strm, pBuffer, iMaxBufferSize);
    if (asn1SccRawData_Encode(pSrc, &strm, &errorCode, TRUE) == FALSE) {
#ifdef __unix__
	fprintf(stderr, "Could not encode RawData (at %s, %d), errorCode was %d\n", __FILE__, __LINE__, errorCode);
#endif
        return -1;
    } else {
        return BitStream_GetLength(&strm);
...
#ifdef __NEED_RawData_ACN
int Encode_ACN_RawData(void *pBuffer, size_t iMaxBufferSize, asn1SccRawData *pSrc)
{
    (void)iMaxBufferSize;
    int errorCode;
    STATIC BitStream strm;
    ...
    BitStream_Init(&strm, pBuffer, iMaxBufferSize);
    if (asn1SccRawData_ACN_Encode(pSrc, &strm, &errorCode, TRUE) == FALSE) {
#ifdef __unix__
	fprintf(stderr, "Could not encode RawData (at %s, %d), errorCode was %d\n", __FILE__, __LINE__, errorCode);
#endif
        return -1;
...
#ifdef __NEED_RawData_NATIVE
int Encode_NATIVE_RawData(void *pBuffer, size_t iMaxBufferSize, const asn1SccRawData *pSrc)
{
    (void)iMaxBufferSize;
    memcpy(pBuffer, pSrc, sizeof(asn1SccRawData) );
    return sizeof(asn1SccRawData);
}
#endif

#ifdef __NEED_RawData_UPER
int Decode_UPER_RawData(asn1SccRawData *pDst, void *pBuffer, size_t iBufferSize)
{
    (void)iBufferSize;
    int errorCode;
    ...
    if (asn1SccRawData_Decode(pDst, &strm, &errorCode)) {
        /* Decoding succeeded */
        return 0;
    } else {
#ifdef __unix__
	fprintf(stderr, "Could not decode RawData (at %s, %d), error code was %d\n", __FILE__, __LINE__, errorCode);
#endif
        return -1;
    }
}
...
#ifdef __NEED_RawData_ACN
int Decode_ACN_RawData(asn1SccRawData *pDst, void *pBuffer, size_t iBufferSize)
{
    (void)iBufferSize;
    int errorCode;
    ...
    if (asn1SccRawData_ACN_Decode(pDst, &strm, &errorCode)) {
        /* Decoding succeeded */
        return 0;
    } else {
#ifdef __unix__
...
#ifdef __NEED_RawData_NATIVE
int Decode_NATIVE_RawData(asn1SccRawData *pDst, void *pBuffer, size_t iBufferSize)
{
    (void)iBufferSize;
    *pDst = *(asn1SccRawData *) pBuffer;
    {
        return 0;
    }
}
