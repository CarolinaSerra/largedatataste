...
{
	return memcmp(pVal1->arr, pVal2->arr, 12583040) ==0	;
...
flag asn1SccRawData_Encode(const asn1SccRawData* pVal, BitStream* pBitStrm, int* pErrCode, flag bCheckConstraints)
{
	...
	if (ret) {
	    ...
	    if (ret) {
	        //encode remaining 128 items
	        BitStream_AppendBit(pBitStrm, 1);
	        BitStream_EncodeConstraintWholeNumber(pBitStrm, 128, 0, 0x7FFF);
	        for(i1=(int)nCurOffset1; i1 < (int)(nCurOffset1 + 128); i1++)
	        {
	        	BitStream_AppendByte0(pBitStrm, pVal->arr[i1]);
	        }
...
flag asn1SccRawData_Decode(asn1SccRawData* pVal, BitStream* pBitStrm, int* pErrCode)
{
	...
	*pErrCode = ERR_UPER_DECODE_RAWDATA; 
	...
	if (ret) {
	    //decode remaining 128 items
	    ret = BitStream_DecodeConstraintWholeNumber(pBitStrm, &nRemainingItemsVar1, 0, 0xFFFF);
	    ret = ret && ((0x8000 & nRemainingItemsVar1) > 0) && ( (0x7FFF & nRemainingItemsVar1) == 128);
	    if (ret) {
	        for(i1=(int)nCurOffset1; i1 < (int)(nCurOffset1 + 128); i1++)
	        {
	    	    ret = BitStream_ReadByte(pBitStrm, &(pVal->arr[i1])); 
