...
#include "x86_partition_polyorb_interface.h"
...

{
   ...
   if (x86_partition_sender_trigger_k == current_task) {
      // puts("[TASTE] Sporadic call of image from thread x86_partition_sender_trigger");
      ...
      __po_hi_gqueue_store_out(x86_partition_sender_trigger_k, sender_trigger_local_outport_receiver_image, request); 
      __po_hi_send_output(x86_partition_sender_trigger_k, sender_trigger_global_outport_receiver_image);

      return;
   }
   // calling threads: sender_trigger partition: x86_partition
}
